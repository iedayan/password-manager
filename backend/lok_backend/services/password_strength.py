...
class PasswordStrengthAnalyzer:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'common_substitutions': re.compile(r'[@4aA][sS5$][sS5$][wW][oO0][rR][dD]', re.IGNORECASE)
        }
    ...
    def _calculate_entropy(self, password: str) -> float:
        ...
        if char_sets['special']: charset_size += 32
        if char_sets['unicode']: charset_size += 100  # Approximate
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        char_sets = analysis['character_sets']
        missing_sets = []
        if not char_sets['lowercase']: missing_sets.append('lowercase letters')
        if not char_sets['uppercase']: missing_sets.append('uppercase letters')
        if not char_sets['digits']: missing_sets.append('numbers')
        if not char_sets['special']: missing_sets.append('special characters')
...
def calculate_password_strength(password: str) -> int:
    ...
    return password_analyzer.analyze_password(password)['score']